    ...
    const fetchMatchedStocks = async () => {
      try {
        ...
        try {
          ...
          logger.warn("[matched stocks] primary endpoint failed", {
            message: err?.message,
            apiUrl,
          });
          ...
          if (!isPerOrderMode && err?.message?.includes("404")) {
            try {
              ...
              logger.log("[matched stocks] using legacy endpoint");
            } catch (err2: any) {
              logger.error(
                "[matched stocks] fallback endpoint failed",
                err2?.message,
              );
        ...
        logger.log("[matched stocks] summary", {
          fetchId,
          isPerOrderMode,
        ...
        const transformedStocks = matches
          .map((item: any, idx: number) => {
            logger.log("[proposal item]", {
              index: idx,
              id: item?.id,
              orderId: item?.order_id ?? item?.order?.id,
              stockId: item?.stock_id ?? item?.stock?.id,
              quantity: item?.quantity_proposed ?? item?.quantity,
              status: item?.status,
              farmerId:
                item?.stock?.farmer?.user?.id ?? item?.stock?.farmer?.id,
            });
            ...
            if (!item.id) {
              logger.warn("[proposal item] missing id", {
                index: idx,
                orderId: item?.order_id ?? order?.id,
                stockId: item?.stock_id ?? stock?.id,
              });
            }
            ...
            return {
              ...
              pricePerKg:
                stock.price_per_kg != null ? Number(stock.price_per_kg) : null,
                ...
                <View style={styles.invoiceRow}>
                  <View>
                    ...
                    <Text style={styles.invoiceSubLabel}>
                      Inc. service & delivery
                    </Text>
                  </View>
                  <Text style={styles.invoiceTotalValue}>
                    Rs.{" "}
                    {item.pricing.estimatedTotal.toLocaleString("en-US", {
