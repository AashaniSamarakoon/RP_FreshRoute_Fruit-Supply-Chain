...
export default function JobDetails() {
  ...
  const openGoogleMaps = () => {
    ...
    // Standard Universal Google Maps URL
  ...
  const handleVerifyQuality = async (orderId: string) => {
    try {
      ...
      // 2. Save it to AsyncStorage so it survives screen refreshes
    ...
    /* // --- KEEP ORIGINAL CODE COMMENTED OUT FOR LATER ---
  const pickupStop = manifest.find(
    (stop) => stop.type === "PICKUP" && stop.order_id === orderId,
  );
  if (!pickupStop) {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (response.success) {
                // Re-fetch the job details to get the updated manifest
                // This will automatically re-render the UI and hide the completed buttons
            ...
            <View key={index} style={styles.stopItem}>
              ...
              <View style={styles.stopContent}>
                ...
                <View style={styles.stopButtons}>
                  ...
                  {/* 1. If it's already completed, show the green success state */}
                  {stop.is_completed ? (
                  ...
                  ) : /* 2. If it's a Pickup but NOT verified yet, show Verify button */
                  stop.type === "PICKUP" &&
                    !verifiedOrders.has(stop.order_id) ? (
                    <TouchableOpacity
                    ...
                    /* 3. Otherwise, show the Confirm action button */
                    <TouchableOpacity
