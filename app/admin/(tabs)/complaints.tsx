...
export default function AdminComplaints() {
  ...
  const fetchComplaints = async (silent = false) => {
    try {
      ...
      const data: any = await api.get("/api/admin/complaints");
      const list: AdminComplaintListItem[] = Array.isArray(data?.complaints)
        ? data.complaints
        : Array.isArray(data)
          ? data
          : data?.data ?? [];
      ...
      if (is403) {
        setForbiddenMessage(
          "You don't have permission to view complaints. Ensure your account has admin access.",
        );
      }
      setComplaints([]);
    } finally {
  ...
  const handleViewDetails = (item: AdminComplaintListItem) => {
    router.push({
      pathname: "/admin/complaint/[id]" as any,
      params: { id: item.id },
    });
  };

  const renderCard = ({ item }: { item: AdminComplaintListItem }) => {
    ...
    const orderId =
      item.placed_order_id ?? item.order_id ?? item.id ?? "";
          ...
          ListEmptyComponent={
            <View style={styles.stateView}>
              ...
              <Text style={styles.emptyTitle}>
                {forbiddenMessage ? "Permission denied" : "No complaints found"}
              </Text>
              <Text style={styles.emptySubtitle}>
                {forbiddenMessage ??
                  (activeTab === "all"
                    ? "Complaints from buyers will appear here."
